  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/mpesa/withdraw`,
        {
          method: "POST",
          headers: {
      ...
      <form className="space-y-6" onSubmit={handleSubmit}>
        <div className="bg-gray-100 dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-xl p-6 shadow-md w-full">
          <h2 className="text-lg font-semibold mb-4 border-b pb-2 text-gray-900 dark:text-gray-200">
            Withdraw Funds
            <p className="text-xs font-semibold text-green-600 italic p-1 bg-green-100 dark:bg-black/30 rounded-md mt-1">
              Configure B2B payments on Settings Tab to use this section
            </p>
          </h2>
          <h5 className="font-semibold mb-4 border-b pb-2 text-gray-900 dark:text-gray-300">
