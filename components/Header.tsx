    ...
    return (
        <>
            {platformData.template !== "EldoHub" && (
                <header
                    className={`fixed top-0 left-0 w-full z-50 transition-colors duration-300 ${isScrolled ? "dark:bg-black bg-white" : "bg-transparent"
                        }`}
                >
                    <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
                        <h1 className="text-black dark:text-white text-3xl font-bold tracking-wide">{platformData?.name || 'NOVA'}</h1>
                        <button
