                    ...
                    return (
                        <div key={station.id} className="bg-gray-800 border border-gray-700 rounded-lg shadow-md hover:shadow-xl transition-all p-6">
                            ...
                            <div className="flex flex-row items-center justify-between">
                                ...
                                <Link target="_blank" rel="noopener noreferrer" href={`https://${station.mikrotikWebfigHost}`} title={`https://${station.mikrotikWebfigHost}`}>
                                    <p className="text-blue-600 semibold underline text-sm flex flex-row items-center gap-1">Open Webfig
                                        <Link2 size={16} />
                                    </p>
                                </Link>
                            </div>
                            <div className="flex flex-col">
                                <p className="text-sm text-gray-500"><span className="text-green-400 font-semibold"> Internal IP: </span>{station.mikrotikHost}</p>
                                <p className="text-sm text-gray-500"><span className="text-green-400 font-semibold"> Remote IP/Port: </span> 51.21.158.217:{getMappedPort(station.mikrotikHost)}</p>
                            </div>

                            <p title={message} className={`mt-2 text-sm font-medium ${status === "Connected" ? "text-green-600" :
