...
export default function Templates() {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto mt-14 min-h-screen text-gray-100">
      <div className="flex justify-between flex-wrap">
        <h1 className="text-3xl font-bold mb-6">Templates</h1>
        <button
          onClick={handleSave}
          disabled={!selectedId}
          className="mb-5 px-4 py-2 bg-blue-600 text-white rounded-lg disabled:bg-gray-700 disabled:cursor-not-allowed hover:bg-blue-500 transition"
        >
        ...
        {templates.map((t) => {
          ...
          return (
            ...
            >
              ...
              <div className="absolute inset-0 bg-black/20 bg-opacity-0 hover:bg-opacity-30 transition">
                <button
                  onClick={(e) => {
                    e.stopPropagation()
                    setExpanded(t.id)
                  }}
                  className="absolute top-2 right-2 bg-gray-700 rounded-full p-1 transition-opacity text-gray-200"
                  title="Expand"
                >
                  <Expand size={16} />
                </button>
              ...
              <div className="absolute bottom-0 left-0 w-full bg-gray-900 bg-opacity-80 text-center py-1">
      ...
      {expanded && (
        <div className="fixed inset-0 bg-black bg-opacity-80 z-50 flex items-center justify-center">
          <div className="relative w-full h-full bg-gray-900">
            <button
              onClick={() => setExpanded("")}
              className="absolute top-4 right-4 bg-gray-800 rounded-full p-2 text-gray-200 hover:bg-gray-700 transition"
            >
              <X size={24} />
            </button>
            <iframe
